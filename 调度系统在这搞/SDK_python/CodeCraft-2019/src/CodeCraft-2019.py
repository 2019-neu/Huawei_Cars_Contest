...
class Car:
    def __init__(self, carid, ffrom, to, speed, planTime):
        ...
        # 车辆总调度状态
        self.status = 0  # 0代表未开始调度，1代表正在调度，2完成调度
        self.end_time = None  # 车辆调度时间

        # 判别器更新信息
        # 车辆物理位置信息存储在车辆类里
        self.road = None  # 道路指针,指示当前道路
        self.sub_road_direction = None  # 在路线的哪个方向
        self.sub_road_id = None  # 在哪个子路线上
        self.sub_road_position = None  # 在子路线的位置
        self.in_road_order = None  # 在道路上的调度顺序ID
        # 车辆前车信息
    ...
    def cal_dis_cross(self):
        return self.road.length - self.subRoadPosition - 1

    def cal_in_road_order(self):
        return self.subRoadId + self.subRoadPosition * 1000

    def update_position(self, subRoadPosition, subRoadId, subRoadDirection, roadId):
        self.sub_road_position = subRoadPosition
        self.sub_road_id = subRoadId
        self.sub_road_direction = subRoadDirection
        self.road = roadId
        self.in_road_order = self.cal_in_road_order()
...
class Road:
    ...
    def add_car(self, car):#加入进入此道路的车
        # 比较Car的in_road_order存入in_road_car_list
        # car.update_position(car.sub_road_position, car.subRoadId, car.subRoadDirection, car.roadId)
        this_num=0
        for road_car in self.in_road_car_list[::-1]:
            if road_car.sub_road_id == car.sub_road_id:
                car.forward = road_car

        for road_car in self.in_road_car_list:
            if car.in_road_order<road_car.in_road_order:
                this_num+=1
        self.in_road_car_list.insert(this_num,car)

    def remove_car(self, carId):#将出了此道路的车移除
        # 遍历in_road_car_lis
        for road_car in self.in_road_car_list:
            if road_car.carid == carId:
                self.in_road_car_list.remove(road_car)

    def get_last_car(self):#得到道路的子道路最后位置的汽车，如果某个子道路没有车，则不返回这个子道路的最后位置车辆
        every_num_id=[]
        every_num=[]
        for i in self.in_road_car_list[::-1]:
            if i.sub_road_id not in every_num:
                every_num_id.append(i.sub_road_id)
                every_num.append(i)
        return every_num

    def update_car_in_road(self, car):#更新可以确定终止状态的汽车，及前车距离大于当前车速
        if car.forward and car.sub_road_position-car.forward.sub_road_position-1>=car.speed:
            car.update_position(car.sub_road_position+car.speed, car.sub_road_id, car.sub_road_position, car.road)
            car.kind=1
...
def main():
    ...
    while all_cross:
        ...
        vlist = dijkstra.dj(one_cross.cid, vlist, vset, edges)
    ...
    while city.all_cars:
        ...
        li=dijkstra.real_get_traj(one_car, one_car.tp, all_list[one_car.ffrom], edges)
        if len(li)<=10:
            ci+=1
        file_result.append(li)
        # print(file_result)
    # print(ci)
