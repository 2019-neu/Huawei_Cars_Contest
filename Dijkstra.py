...
while city.all_roads:
	...
	if every_road.dir==1:
		add_edge(int(every_road.cid1), int(every_road.cid2), int(every_road.length))
		add_edge(int(every_road.cid2), int(every_road.cid1), int(every_road.length))
	...
	while(len(mvset)!=0):
		...
		for w in v.outList:#w为索引
			...
			if(mvlist[w].dist == float('inf')):
				...
				mvlist[w].dist = v.dist + edges[(v.vid,w)]
				mvlist[w].prev = v.vid
			else:
				if((v.dist + edges[(v.vid,w)])<mvlist[w].dist):
					mvlist[w].dist = v.dist + edges[(v.vid,w)]
					mvlist[w].prev = v.vid
				else:#原路径长更小，没有必要更新
					pass

def real_get_traj(start,index, vvlist):
	traj_list = []
	def get_traj(index):#参数是顶点在vlist中的索引
		if(index == start.ffrom):#终点
			traj_list.append(index)
			traj_list.append(start.plan_time)
...
cmpfun = operator.attrgetter('plan_time') #安装plan_time进行排序
city.all_cars.sort(key = cmpfun)
print("Quick Sort is ok! ")

cont=0
for i in range(len(city.all_cars)):
	city.all_cars[i].plan_time=city.all_cars[i].plan_time+int((cont/1000))
	cont+=1
